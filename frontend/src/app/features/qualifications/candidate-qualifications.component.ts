    ...
    <div [class.page-container]="!embedded" [class.step-body-padded]="embedded">
      @if (!embedded) {
        <div class="page-header">
          <div>
            <h2 class="page-title">
              <i class="ti ti-school"></i> Qualifications
            </h2>
            <p class="page-sub">
              Education and certifications, visible to recruiters
            </p>
          </div>
        </div>
      }
      ...
      @if (autofillStore.qualifications().length) {
        <div class="autofill-review-block">
          <div class="autofill-review-header">
            ...
            {{ autofillStore.qualifications().length }} qualification(s) in your
            CV — review and add
          </div>
            ...
            <div class="autofill-card autofill-card-stack">
              <div class="autofill-card-body">
                ...
                <div class="autofill-card-sub">
                  {{ item.qualificationType }} · {{ item.institution }}
                </div>
                @if (item.yearCompleted) {
                  <div class="autofill-card-desc">
                    Completed {{ formatYear(item.yearCompleted) }}
                  </div>
                } @else {
                  <input
                    type="date"
                    ...
                    (change)="
                      qualYearDraft[item.id] = $any($event.target).value
                    "
                  />
                  <span class="autofill-missing-note"
                    >AI couldn't find a date — set one to add this.</span
                  >
                }
              </div>
              <div class="autofill-card-actions">
                <button
                  ...
                  (click)="addSuggestedQualification(item)"
                  [disabled]="
                    saving() || (!item.yearCompleted && !qualYearDraft[item.id])
                  "
                >
                ...
                <button
                  type="button"
                  class="chip-dismiss"
                  (click)="autofillStore.removeQualification(item.id)"
                >
                  <i class="ti ti-x"></i>
                </button>
              </div>
      ...
      >
        <mat-card-content style="padding:18px 20px">
          <div class="form-section-label">
            <i class="ti ti-plus"></i> Add a qualification
          </div>
          <form [formGroup]="form" (ngSubmit)="add()">
            ...
            <div class="attach-row">
              ...
              >
                <i class="ti ti-paperclip"></i>&nbsp;{{
                  selectedFile()
                    ? 'Change file'
                    : 'Attach certificate / transcript (optional)'
                }}
              </button>
              @if (selectedFile()) {
            ...
            <p class="form-note" style="margin-top:4px">
              <i class="ti ti-info-circle"></i> PDF, DOC, DOCX · max 5 MB. You
              can add this later if you don't have it on hand.
            </p>
          ...
          @for (q of qualifications(); track q.candidateQualificationId) {
            <div class="doc-slot">
              <div class="doc-info">
                ...
                <div class="doc-meta">
                  {{ q.qualificationType }} · {{ q.institution }} ·
                  {{ formatYear(q.yearCompleted) }}
                </div>
                @if (attachmentFor(q.candidateQualificationId); as att) {
                  <a
                    [href]="fileUrl(att.fileUrl)"
                    target="_blank"
                    class="attach-link"
                  >
                  ...
                  <span class="attach-missing"
                    ><i class="ti ti-paperclip-off"></i> No certificate
                    attached</span
                  >
                }
              </div>
              <div class="doc-actions">
                <button class="btn-remove" (click)="remove(q)">
...
export class CandidateQualificationsComponent implements OnInit {
  ...
  ): CandidateDocumentResponse | undefined {
    return this.attachments().find(
      (a) => a.qualificationId === qualificationId,
    );
  }
    ...
    this.qualService
      ...
      .subscribe({
        next: (created) => {
          ...
          const docType: DocumentTypeKey =
            created.qualificationType === 'Certification'
              ? 'Certification'
              : 'Qualification';
          ...
          this.docService
            .upload(
              ...
              docType,
              file,
              created.candidateQualificationId,
            )
            .subscribe({
              next: (uploaded) => {
                this.attachments.update((list) => [...list, uploaded]);
                this.finishAdd(
                  'Qualification added with certificate attached.',
                  wasEmpty,
                );
              },
              error: (err: Error) => {
                this.finishAdd(
                  `Qualification added, but the file failed to attach: ${err.message}`,
                  wasEmpty,
                );
              },
            });
  ...
  remove(q: QualificationResponse): void {
    ...
    if (
      !confirm(
        `Remove "${q.name}"? Any attached certificate will be removed too.`,
      )
    )
      return;
    this.qualService
      .delete(p.candidateId, q.candidateQualificationId)
      .subscribe({
        next: () => {
          this.qualifications.update((list) =>
            list.filter(
              (x) => x.candidateQualificationId !== q.candidateQualificationId,
            ),
          );
          this.attachments.update((list) =>
            list.filter(
              (a) => a.qualificationId !== q.candidateQualificationId,
            ),
          );
    ...
    this.qualService
      ...
      .subscribe({
        next: (created) => {
          ...
          if (this.qualifications().length === 1 && this.autoAdvanceOnSave)
            this.saved.emit();
        },
        error: (err: Error) => {
          this.saving.set(false);
