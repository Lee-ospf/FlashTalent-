...
@Component({
  ...
  template: `
          ...
          <mat-card class="mat-elevation-z1 ad-card">
            ...
            @if (candidate(); as c) {
              <div class="kv-grid" style="margin-top:10px">
                <div class="kv">
                  <span class="kv-icon"><i class="ti ti-mail"></i></span>
                  <div>
                    <span class="kv-label">Email</span
                    ><span class="kv-val">{{ c.email }}</span>
                  </div>
                </div>
                <div class="kv">
                  <span class="kv-icon"><i class="ti ti-phone"></i></span>
                  <div>
                    <span class="kv-label">Phone</span
                    ><span class="kv-val">{{ c.phone || '—' }}</span>
                  </div>
                </div>
                <div class="kv">
                  <span class="kv-icon"
                  ...
                  <div>
                    ...
                    ><span class="kv-val">{{ c.gender || '—' }}</span>
                  </div>
                </div>
                <div class="kv">
                  <span class="kv-icon"><i class="ti ti-flag"></i></span>
                  <div>
                    <span class="kv-label">Nationality</span
                    ><span class="kv-val">{{ c.nationality || '—' }}</span>
                  </div>
                </div>
                <div class="kv">
                  <span class="kv-icon"><i class="ti ti-cake"></i></span>
                  <div>
                ...
                @for (e of experience(); track e.candidateExperienceId) {
                  <div class="exp-row">
                    ...
                    <div class="exp-body">
                      <div class="exp-role">
                        {{ e.role }}
                        <span class="exp-at">at {{ e.company }}</span>
                      </div>
                      <div class="exp-dates">
                      ...
                      @if (e.projectsAndDuties) {
                        <div class="exp-notes">{{ e.projectsAndDuties }}</div>
                      }
                    </div>
                  </div>
                }
                ...
                @for (q of qualifications(); track q.candidateQualificationId) {
                  <div class="qual-row">
                    <span class="qual-icon">
                      <i
                        class="ti"
                        [class.ti-certificate]="
                          q.qualificationType === 'Certification'
                        "
                        [class.ti-books]="
                          q.qualificationType !== 'Certification'
                        "
                    ...
                    <div>
                      ...
                      <div class="qual-sub">
                        {{ q.institution }} · {{ formatDate(q.yearCompleted) }}
                      </div>
                    </div>
                  </div>
                }
              ...
              <div class="skill-chips" style="margin-top:10px">
                @for (s of skills(); track s.candidateSkillId) {
                  <span class="skill-chip"
                    ><span
                      class="skill-dot"
                      [ngClass]="skillClass(s.proficiencyLevel)"
                    ></span
                    >{{ s.skillName
    ...
    <style>
      ...
      .ad-card ::ng-deep .mat-mdc-card-content {
        padding: 0;
      }
      ...
      .skill-dot.lvl-expert {
        background: var(--green);
      }
      .skill-dot.lvl-intermediate {
        background: var(--blue);
      }
      .skill-dot.lvl-beginner {
      ...
      next: (app) => {
        ...
        forkJoin({
          candidate: this.candidateService
            .getById(app.candidateId)
            .pipe(catchError(() => of(null))),
          skills: this.skillService
            .getAll(app.candidateId)
            .pipe(catchError(() => of([]))),
          experience: this.experienceService
            .getAll(app.candidateId)
            .pipe(catchError(() => of([]))),
          qualifications: this.qualificationService
            .getAll(app.candidateId)
            .pipe(catchError(() => of([]))),
