    ...
    <div class="page-container">
      <div class="page-header">
        <div>
          ...
          <p class="page-sub">
            Recruitment opportunities are created in Draft and stay hidden from
            candidates until published
          </p>
        </div>
        @if (vacancyId) {
          <span
            class="status-pill"
            [class.s-applied]="status() === 'Draft'"
            [class.s-offer]="status() === 'Published'"
          >
            {{ status() }}
          </span>
        }
      ...
      @if (status() && status() !== 'Draft') {
        <div class="info-banner warn">
          ...
          This vacancy is {{ status() }} and can no longer be edited — only
          Draft vacancies are editable.
        </div>
      }
      ...
      @if (loading()) {
        ...
        <form [formGroup]="form" (ngSubmit)="save('draft')">
          ...
          >
            <mat-card-content style="padding:18px 20px">
              <div class="form-section-label">
                <i class="ti ti-info-circle"></i> Vacancy details
              </div>
              ...
              <mat-form-field appearance="outline" style="width:100%">
                ...
                />
                @if (invalid('title')) {
                  <mat-error>Vacancy title is required</mat-error>
                }
              </mat-form-field>
              ...
              <div class="field-grid">
                  ...
                  <mat-form-field appearance="outline" style="width:100%">
                    ...
                    <mat-select formControlName="departmentId">
                      @for (d of departments(); track d.departmentId) {
                        <mat-option [value]="d.departmentId">{{
                          d.name
                        }}</mat-option>
                      }
                    </mat-select>
                    @if (invalid('departmentId')) {
                ...
                <mat-form-field appearance="outline" style="width:100%">
                  ...
                  @if (form.errors?.['salaryRange']) {
                    <mat-error>Max salary must be greater than min</mat-error>
                  }
                </mat-form-field>
                ...
                <mat-form-field appearance="outline" style="width:100%">
                  ...
                  <input
                    ...
                    [min]="minClosingDate"
                  />
                  @if (invalid('closingDate')) {
                    <mat-error
                      >Closing date is required and must be in the
                      future</mat-error
                    >
                  }
                </mat-form-field>
              ...
              <mat-form-field appearance="outline" style="width:100%">
                ...
                ></textarea>
                @if (invalid('requiredQualifications')) {
                  <mat-error>Qualifications are required</mat-error>
                }
              </mat-form-field>
          ...
          >
            <mat-card-content style="padding:18px 20px">
              <div class="form-section-label">
                <i class="ti ti-tools"></i> Required skills
              </div>
              @if (skillsArray.length === 0) {
                <p class="form-note" style="margin-bottom:10px">
                  <i class="ti ti-alert-circle"></i> At least one skill is
                  required
                </p>
              }
                  ...
                  >
                    <mat-form-field appearance="outline" style="width:220px">
                      ...
                      <mat-select formControlName="skillId">
                        @for (
                          s of availableSkillsFor($index);
                          track s.skillId
                        ) {
                          <mat-option [value]="s.skillId">
                            {{ s.name }}
                            <span style="opacity:.5">· {{ s.category }}</span>
                          </mat-option>
                        }
                      </mat-select>
                    </mat-form-field>
              ...
              <button
                ...
                [disabled]="skillsArray.length >= skills().length"
                (click)="addSkill()"
              >
                <i class="ti ti-plus"></i> Add skill
              </button>
              @if (
                skillsArray.length >= skills().length && skills().length > 0
              ) {
                <p class="form-note" style="margin-top:6px">
                  <i class="ti ti-info-circle"></i> All available skills have
                  been added
                </p>
              }
            </mat-card-content>
          ...
          >
            <mat-card-content style="padding:18px 20px">
              <div class="form-section-label">
                <i class="ti ti-files"></i> Required documents (optional)
              </div>
              <div style="display:flex;flex-direction:column;gap:8px">
                @for (t of allDocTypes; track t) {
                  <div style="display:flex;align-items:center;gap:10px">
                    <mat-checkbox
                      [checked]="isDocSelected(t)"
                      (change)="toggleDoc(t, $event.checked)"
                    >
                      {{ docLabel(t) }}
                    </mat-checkbox>
                    @if (isDocSelected(t)) {
                      <mat-checkbox
                        [checked]="isDocMandatory(t)"
                        (change)="setDocMandatory(t, $event.checked)"
                      >
                        Mandatory
                      </mat-checkbox>
                    }
          ...
          <div class="form-footer">
            <a routerLink="/admin/vacancies" class="form-note"
              ><i class="ti ti-arrow-left"></i> Back to vacancies</a
            >
            <div style="display:flex;gap:8px">
              ...
              >
                ...
                {{ vacancyId ? 'Save changes' : 'Save as draft' }}
              </button>
            </div>
          </div>
        </form>
...
export class CreateVacancyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.departmentService.getAll().subscribe({
      next: (d) => this.departments.set(d.filter((x) => x.isActive)),
    });
      ...
      this.vacancyService.getById(this.vacancyId).subscribe({
        next: (v) => {
          ...
          this.form.patchValue({
            ...
            vacancyType: v.vacancyType as any,
          ...
          v.skills?.forEach((s) => {
            this.skillsArray.push(
              this.fb.group({
                skillId: [s.skillId, Validators.required],
  ...
  availableSkillsFor(currentIndex: number): SkillResponse[] {
    const takenIds = new Set(
      this.skillsArray.controls
        .map((c, i) => (i !== currentIndex ? c.get('skillId')?.value : null))
        .filter((id) => id !== null && id !== undefined),
    );
    return this.skills().filter(
      (s) =>
        !takenIds.has(s.skillId) ||
        s.skillId === this.skillsArray.at(currentIndex).get('skillId')?.value,
    );
  }
  ...
  isDocMandatory(t: string): boolean {
    return (
      this.docsArray.controls.find((c) => c.value.documentType === t)?.value
        .isMandatory ?? false
    );
  }
  toggleDoc(t: string, checked: boolean): void {
    if (checked) {
      this.docsArray.push(
        this.fb.group({ documentType: [t], isMandatory: [false] }),
      );
    } else {
      const idx = this.docsArray.controls.findIndex(
        (c) => c.value.documentType === t,
      );
      if (idx > -1) this.docsArray.removeAt(idx);
    }
  }
  setDocMandatory(t: string, checked: boolean): void {
    const ctrl = this.docsArray.controls.find(
      (c) => c.value.documentType === t,
    );
    ctrl?.patchValue({ isMandatory: checked });
  }
  docLabel(t: string): string {
    return (DOCUMENT_TYPE_LABELS as Record<string, string>)[t] ?? t;
  }
  ...
  save(mode: 'draft' | 'publish'): void {
    ...
    if (!this.recruiterId) {
      this.apiError =
        'Your recruiter profile could not be found. Contact an admin.';
      return;
    }
    ...
    const payload = {
      ...
      departmentId:
        v.vacancyType === 'Internal'
          ? (v.departmentId ?? undefined)
          : undefined,
      clientId:
        v.vacancyType === 'ClientPlacement'
      ...
      location: v.location!,
      closingDate: new Date(v.closingDate!).toISOString(),
      ...
      if (mode === 'publish') {
        this.vacancyService.publish(id).subscribe({
          next: () => {
            ...
            this.toast.show(
              'Vacancy published — now visible to candidates.',
              'success',
            );
    ...
    if (this.vacancyId) {
      ...
      this.vacancyService
        .create({ recruiterId: this.recruiterId, ...payload })
        .subscribe({
