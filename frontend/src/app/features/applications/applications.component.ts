...
import { PrescreeningService, PrescreeningResponse, PrescreeningTemplateResponse, validatePrescreeningFile } from '../../core/services/prescreening.service';
...
const PIPELINE = ['Applied', 'UnderReview', 'Shortlisted', 'PrescreeningStage', 'InterviewStage', 'OfferExtended', 'Hired'];
...
const STATUS_LABEL: Record<string, string> = {
  ...
  PrescreeningStage:'Pre-Screening', InterviewStage:'Interview',
  OfferExtended:'Offer Extended', Hired:'Hired', NotSelected:'Not Selected',
  OfferSent:'Offer Sent', OfferAccepted:'Offer Accepted', OfferDeclined:'Offer Declined'
};
>>>>>>> b6ba2b1b7481624003e23123f272975cd6832d64
...
=======
              <div class="app-card-header" (click)="toggle(app.applicationId)"
                ...
                <div class="app-icon-wrap app-icon-{{ statusClass(effectiveStatus(app)) }}">
>>>>>>> b6ba2b1b7481624003e23123f272975cd6832d64
...
<<<<<<< HEAD
                ...
                ></i>
=======
                <span class="status-pill s-{{ statusClass(effectiveStatus(app)) }}">{{ statusLabel(effectiveStatus(app)) }}</span>
                  ...
                  <div style="padding:18px 20px">
                    ...
                    @if (prescreeningDoc(app.applicationId); as doc) {
                      <div style="margin-bottom:20px">
                        <div class="ps-section-label">Pre-screening assessment</div>

                        @if (doc.status === 'Sent') {
                          <p class="ps-intro">A pre-screening document has been sent for this application. Download it, fill it in, then upload the completed file below.</p>
                          <div class="ps-actions">
                            @if (template(); as t) {
                              <a class="btn-secondary" [href]="fileHref(t.fileUrl)" target="_blank" rel="noopener">
                                <i class="ti ti-eye"></i> View document
                              </a>
                              <a class="btn-secondary" [href]="fileHref(t.fileUrl)" [download]="t.originalFileName">
                          ...
                          <div class="ps-upload-box">
                            <input type="file" class="ps-file-input" accept=".pdf,.doc,.docx"
                                   [id]="'ps-file-' + app.applicationId"
                                   (change)="onFileSelected($event, app.applicationId)">
                            <label class="btn-secondary" [for]="'ps-file-' + app.applicationId">
                              <i class="ti ti-paperclip"></i>
                              {{ selectedFileName(app.applicationId) || 'Choose completed document' }}
                            </label>
                            @if (uploadError(app.applicationId)) {
                              <div class="api-error" style="margin-top:8px">
                                <i class="ti ti-alert-circle"></i> {{ uploadError(app.applicationId) }}
                              </div>
                            }
                            <div class="ps-actions" style="margin-top:10px">
                              <button class="btn-primary" [disabled]="!selectedFileName(app.applicationId) || submittingId() === app.applicationId"
                          ...
                          <div class="ps-doc-row">
                            <span class="ps-doc-icon"><i class="ti ti-circle-check"></i></span>
                            <div class="ps-doc-meta">
                              <div class="ps-doc-name">{{ doc.completedOriginalFileName || 'Assessment submitted' }}</div>
                              <div class="ps-doc-sub">Submitted {{ formatDate(doc.submittedAt!) }} · the recruiter can now view it</div>
                            </div>
                            <div style="display:flex;gap:8px;flex-shrink:0">
                              <a class="btn-secondary doc-view-btn" [href]="fileHref(doc.completedFileUrl!)" target="_blank" rel="noopener">
                    ...
                    @if (offerDoc(app.applicationId); as offerLetter) {
                      <div style="margin-bottom:20px">
                        ...
                        <div class="offer-kv-grid">
                          <div class="offer-kv"><span class="offer-kv-label">Position</span><span class="offer-kv-val">{{ offerLetter.jobTitle }}</span></div>
                          <div class="offer-kv"><span class="offer-kv-label">Employment type</span><span class="offer-kv-val">{{ offerLetter.employmentType || '—' }}</span></div>
                          <div class="offer-kv"><span class="offer-kv-label">Location</span><span class="offer-kv-val">{{ offerLetter.location || '—' }}</span></div>
                          <div class="offer-kv"><span class="offer-kv-label">Salary</span><span class="offer-kv-val">ZAR {{ offerLetter.salary.toLocaleString('en-ZA') }}</span></div>
                          <div class="offer-kv"><span class="offer-kv-label">Proposed start date</span><span class="offer-kv-val">{{ formatDate(offerLetter.startDate) }}</span></div>
                          <div class="offer-kv"><span class="offer-kv-label">Closing date</span><span class="offer-kv-val">{{ formatDate(offerLetter.closingDate) }}</span></div>
                        </div>

                        <div class="ps-actions" style="margin-top:12px">
                          <button class="btn-secondary" (click)="viewOffer(offerLetter)">
                            <i class="ti ti-eye"></i> View offer letter
                          </button>
                          <button class="btn-secondary" (click)="downloadOffer(offerLetter)">
                        ...
                        @if (offerLetter.status === 'Sent') {
                          <div class="offer-response-box">
                            <p class="ps-intro" style="margin-bottom:12px">Review the terms above, then accept or decline this offer.</p>

                            @if (!isDeclining(app.applicationId)) {
                              <div class="ps-actions">
                                <button class="btn-primary" [disabled]="respondingId() === app.applicationId" (click)="acceptOffer(app)">
                                  @if (respondingId() === app.applicationId) { <mat-spinner diameter="14" style="display:inline-block;margin-right:6px"></mat-spinner> }
                                  <i class="ti ti-circle-check"></i> Accept offer
                                </button>
                                <button class="btn-secondary" [disabled]="respondingId() === app.applicationId" (click)="startDecline(app.applicationId)">
                                  <i class="ti ti-circle-x"></i> Decline offer
                                </button>
                              </div>
                            } @else {
                              <textarea class="assess-comment-plain" rows="2" placeholder="Reason for declining (optional, for your own reference)…"
                                        [ngModel]="declineReasonDraft(app.applicationId)" (ngModelChange)="setDeclineReason(app.applicationId, $event)"></textarea>
                              <div class="ps-actions" style="margin-top:10px">
                                <button class="btn-primary" [disabled]="respondingId() === app.applicationId" (click)="confirmDecline(app)">
                                  @if (respondingId() === app.applicationId) { <mat-spinner diameter="14" style="display:inline-block;margin-right:6px"></mat-spinner> }
                                  Confirm decline
                                </button>
                                <button class="btn-secondary" (click)="cancelDecline(app.applicationId)">Cancel</button>
                              </div>
                            }
                          </div>
                        } @else if (offerLetter.status === 'Accepted') {
                          <p class="form-note" style="color:#1a5c35;margin-top:10px"><i class="ti ti-circle-check"></i> You accepted this offer on {{ formatDate(offerLetter.respondedAt!) }}.</p>
                        } @else {
                          <p class="form-note" style="color:var(--red);margin-top:10px">
                            <i class="ti ti-circle-x"></i> You declined this offer on {{ formatDate(offerLetter.respondedAt!) }}.
                          </p>
                        }
                      </div>
                    }
...
=======
      ...
      <div class="ps-modal-backdrop" (click)="closeOfferPreview()">
        <div class="ps-modal" (click)="$event.stopPropagation()">
          <div class="ps-modal-header">
            <div><i class="ti ti-file-certificate"></i> Offer letter — {{ o.jobTitle }}</div>
            <button class="ps-modal-close" (click)="closeOfferPreview()"><i class="ti ti-x"></i></button>
          </div>
          <div class="ps-modal-body">
            <iframe class="ps-modal-frame" [srcdoc]="o.generatedHtml"></iframe>
          </div>
          <div class="ps-modal-footer">
            <button class="btn-secondary" (click)="closeOfferPreview()">Close</button>
            <button class="btn-primary" (click)="downloadOffer(o)"><i class="ti ti-download"></i> Download</button>
          </div>
        </div>
      </div>
    }
    ...
    <style>
      ...
      .ps-intro { font-size: 13px; color: var(--text); line-height: 1.6; margin-bottom: 12px; }
      .ps-actions { display: flex; gap: 8px; margin-top: 0; flex-wrap: wrap; align-items: center; }

      .ps-upload-box {
        margin-top: 14px; padding: 14px; border: 1px solid var(--border);
        border-radius: 10px; background: #fff;
      }
      .ps-file-input { position: absolute; width: 1px; height: 1px; overflow: hidden; opacity: 0; }
      ...
      .ps-doc-icon {
        ...
        background: var(--green-bg); color: #1a5c35;
        display: flex; align-items: center; justify-content: center; font-size: 15px;
      }
      .ps-doc-meta { flex: 1; min-width: 0; }
      .ps-doc-name { font-size: 13px; font-weight: 700; color: var(--text); word-break: break-word; }
      .ps-doc-sub { font-size: 11px; color: var(--text-muted); margin-top: 2px; }
      .doc-view-btn { padding: 6px 12px; font-size: 12px; flex-shrink: 0; }

      .offer-kv-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0 20px; }
      .offer-kv { display: flex; flex-direction: column; padding: 9px 0; border-bottom: 1px solid var(--border); }
      .offer-kv:nth-last-child(-n+2) { border-bottom: none; padding-bottom: 0; }
      .offer-kv-label { font-size: 10.5px; color: var(--text-muted); font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
      .offer-kv-val { font-size: 13px; color: var(--text); font-weight: 600; margin-top: 3px; }
      .offer-terms { font-size: 12.5px; color: var(--text-muted); line-height: 1.6; margin-top: 12px; white-space: pre-line; }
      .offer-response-box { margin-top: 16px; padding: 14px; border: 1px solid var(--border); border-radius: 10px; background: #fff; }
      ...
      .assess-comment-plain:focus { outline: none; border-color: var(--navy); box-shadow: 0 0 0 3px rgba(26,39,68,0.08); }
      ...
      .ps-modal {
        background: #fff; border-radius: 14px; width: min(680px, 100%); max-height: 86vh;
      ...
      .ps-modal-frame { width: 100%; height: 60vh; border: none; background: #fff; }
...
=======
      next: a => {
        ...
        this.prescreening.preload(a.map(app => app.applicationId)).subscribe();
...
>>>>>>> b6ba2b1b7481624003e23123f272975cd6832d64
  ...
  pipelineSteps(app: ApplicationResponse) {
    ...
    const isNotSelected = status === 'NotSelected' || status === 'OfferDeclined';
    const progressStatus = (status === 'OfferSent' || status === 'OfferAccepted') ? 'OfferExtended' : status;
...
=======
  ...
  onFileSelected(event: Event, applicationId: number): void {
    ...
    if (error) { this.uploadErrors[applicationId] = error; delete this.selectedFiles[applicationId]; return; }
  ...
  submitUpload(app: ApplicationResponse): void {
    ...
    if (!file) {
      this.uploadErrors[app.applicationId] = 'Please choose a completed document first.';
      return;
    }
    ...
    this.prescreening.submit(app.applicationId, file).subscribe({
      next: doc => {
        ...
        this.toast.show('Pre-screening document uploaded. The recruiter can now view it.', 'success');
      },
      error: (err: Error) => {
        this.submittingId.set(null);
  ...
  viewOffer(offer: OfferLetterResponse): void { this.previewingOffer.set(offer); }
  closeOfferPreview(): void { this.previewingOffer.set(null); }

  isDeclining(applicationId: number): boolean { return this.decliningIds().has(applicationId); }
  ...
  cancelDecline(applicationId: number): void {
    this.decliningIds.update(s => { const n = new Set(s); n.delete(applicationId); return n; });
    delete this.declineReasons[applicationId];
  }

  declineReasonDraft(applicationId: number): string { return this.declineReasons[applicationId] ?? ''; }
  setDeclineReason(applicationId: number, value: string): void { this.declineReasons[applicationId] = value; }
  ...
  acceptOffer(app: ApplicationResponse): void {
    ...
    this.offerLetter.respond(offer.offerLetterId, 'Accepted').subscribe({
      next: () => {
        ...
        this.toast.show('Offer accepted! The recruiter has been notified.', 'success');
      },
      error: (err: Error) => { this.respondingId.set(null); this.toast.show(err.message, 'error'); }
    });
  }
