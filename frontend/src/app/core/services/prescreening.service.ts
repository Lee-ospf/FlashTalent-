...
export function validatePrescreeningFile(file: File): string | null {
  ...
  if (!ALLOWED_EXT.includes(ext)) return `"${file.name}" — unsupported type. Accepted: PDF, DOC, DOCX.`;
  if (file.size > MAX_BYTES) return `"${file.name}" is ${(file.size / 1024 / 1024).toFixed(1)} MB — max 5 MB.`;
  return null;
}
...
export class PrescreeningService {
  ...
  getTemplate(): Observable<PrescreeningTemplateResponse | null> {
    return this.http.get<PrescreeningTemplateResponse>(`${this.base}/template`).pipe(
      catchError((err: HttpErrorResponse) => {
        if (err.status === 404) return of(null);
        return throwError(() => new Error(err.error?.message ?? 'Failed to load the pre-screening template.'));
      })
    );
  }
  ...
  uploadTemplate(file: File): Observable<PrescreeningTemplateResponse> {
    ...
    return this.http.post<PrescreeningTemplateResponse>(`${this.base}/template`, form)
  ...
  send(applicationId: number): Observable<PrescreeningResponse> {
    return this.http.post<PrescreeningResponse>(`${this.base}/${applicationId}/send`, {}).pipe(
  ...
  submit(applicationId: number, file: File): Observable<PrescreeningResponse> {
    const form = new FormData();
    form.append('file', file, file.name);
    return this.http.post<PrescreeningResponse>(`${this.base}/${applicationId}/submit`, form).pipe(
      tap(res => this.updateCache(res)),
      catchError(this.handleError)
    );
  }
  ...
  getByApplication(applicationId: number): Observable<PrescreeningResponse | null> {
    return this.http.get<PrescreeningResponse>(`${this.base}/${applicationId}`).pipe(
      tap(res => this.updateCache(res)),
      catchError((err: HttpErrorResponse) => {
        if (err.status === 404) { this.removeFromCache(applicationId); return of(null); }
        return throwError(() => new Error(err.error?.message ?? 'Failed to load pre-screening.'));
      })
    );
  }

  /** Fetches several applications' pre-screening records at once and warms the cache. Errors per-item are swallowed to null. */
  preload(applicationIds: number[]): Observable<(PrescreeningResponse | null)[]> {
    const unique = [...new Set(applicationIds)];
    if (!unique.length) return of([]);
    return forkJoin(unique.map(id => this.getByApplication(id).pipe(catchError(() => of(null)))));
  }

  /** Recruiter/Admin records a Pass/Fail outcome once the candidate has submitted. */
  setOutcome(applicationId: number, outcome: 'Passed' | 'Failed', recruiterNotes?: string): Observable<PrescreeningResponse> {
    return this.http.put<PrescreeningResponse>(`${this.base}/${applicationId}/outcome`, {
      outcome,
  ...
  fileHref(relativeUrl: string): string {
    ...
    return relativeUrl.startsWith('/') ? origin + relativeUrl : `${origin}/${relativeUrl}`;
  }
